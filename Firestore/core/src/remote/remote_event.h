...
class WatchChangeAggregator {
 ...
 private:
   ...
   * filter.
   */
  static absl::optional<BloomFilter> ParseBloomFilter(const ExistenceFilterWatchChange& existence_filter);

  /**
   * Apply bloom filter to remove the deleted documents, and return the
   * application status.
   */
  BloomFilterApplicationStatus ApplyBloomFilter(const BloomFilter& bloom_filter, const ExistenceFilterWatchChange& existence_filter, int current_count);
